<div class="p-4 relative">
    ...
    <div class="absolute top-0 left-0 w-900 h-40 opacity-70 z-20">
        <img src="/images/curvas_arriba.png" alt="Top Curve" class="w-600 h-40 object-cover">
    </div>
    ...
    @if($isOpen)
        <div class="fixed z-50 inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:max-w-2xl sm:w-full z-10" style="background-image: url('/images/icono_central.png'); background-size: contain; background-repeat: no-repeat; background-position: center;">
                ...
                <!-- Parte superior azul con el título centrado -->
                ...
                <div class="bg-white bg-opacity-75 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <input type="text" wire:model="forma_pago" placeholder="Forma de Pago" class="w-full p-2 border rounded-lg shadow-inner bg-blue-50">
                        @error('forma_pago') <span class="text-red-500">{{ $message }}</span> @enderror

                        <input type="text" wire:model="cliente" placeholder="Cliente" class="w-full p-2 border rounded-lg shadow-inner bg-blue-50">
                        @error('cliente') <span class="text-red-500">{{ $message }}</span> @enderror

                        <input type="text" wire:model="nombre_cliente" placeholder="Nombre Cliente" class="w-full p-2 border rounded-lg shadow-inner bg-blue-50">
                        @error('nombre_cliente') <span class="text-red-500">{{ $message }}</span> @enderror

                        <input type="text" wire:model="direccion_cliente" placeholder="Dirección Cliente" class="w-full p-2 border rounded-lg shadow-inner bg-blue-50">
                        @error('direccion_cliente') <span class="text-red-500">{{ $message }}</span> @enderror

                        <input type="text" wire:model="telefono_cliente" placeholder="Teléfono Cliente" class="w-full p-2 border rounded-lg shadow-inner bg-blue-50">
                        @error('telefono_cliente') <span class="text-red-500">{{ $message }}</span> @enderror

                        <!-- Select para seleccionar un producto -->
                        <input type="text" placeholder="Buscar Producto" class="w-full p-2 border rounded-lg shadow-inner bg-blue-50">
                        <select wire:model.live="selectedProduct" class="w-full p-2 border rounded-lg shadow-inner bg-blue-50">
                        ...
                        @error('codigo_articulo') <span class="text-red-500">{{ $message }}</span> @enderror

                        <input type="text" wire:model="codigo_articulo" placeholder="Código Artículo" class="w-full p-2 border rounded-lg shadow-inner bg-blue-50" readonly>

                        <input type="text" wire:model="tipo_unidad" placeholder="Tipo Unidad" class="w-full p-2 border rounded-lg shadow-inner bg-blue-50" readonly>
                        <input type="text" wire:model="valor_unitario" placeholder="Valor Unitario" class="w-full p-2 border rounded-lg shadow-inner bg-blue-50" readonly>
                        <input type="text" wire:model="cantidad" placeholder="Cantidad" class="w-full p-2 border rounded-lg shadow-inner bg-blue-50">
                        @error('cantidad') <span class="text-red-500">{{ $message }}</span> @enderror

                        <input type="text" wire:model="iva" placeholder="IVA" class="w-full p-2 border rounded-lg shadow-inner bg-blue-50">
                        @error('iva') <span class="text-red-500">{{ $message }}</span> @enderror

                        <textarea wire:model="observaciones" placeholder="Observaciones" class="w-full p-2 border rounded-lg shadow-inner bg-blue-50"></textarea>
        ...
        <table class="table-auto w-full border-collapse bg-white shadow-lg rounded-lg overflow-hidden">
            ...
            @foreach($sales as $sale)
                <tr class="border-b hover:bg-blue-100">
                    <td class="px-4 py-2 border-b">{{ $sale->forma_pago }}</td>
                    <td class="px-4 py-2 border-b">{{ $sale->nombre_cliente }}</td>
                    <td class="px-4 py-2 border-b">{{ $sale->nombre_articulo }}</td>
                    <td class="px-4 py-2 border-b">{{ $sale->cantidad }}</td>
                    <td class="px-4 py-2 border-b">{{ $sale->valor_unitario }}</td>
                    <td class="px-4 py-2 border-b">{{ $sale->iva }}</td>
                    <td class="px-4 py-2 border-b flex space-x-2">
                        <!-- Botón de editar con ícono de lápiz -->
                        <button wire:click="edit({{ $sale->id }})" class="bg-green-500 hover:bg-green-700 text-white font-bold py-1 px-3 rounded">
                            <i class="fa-solid fa-pen-to-square"></i>
                        </button>

                        <!-- Botón de eliminar con ícono de basurero -->
                        <button wire:click="delete({{ $sale->id }})" class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-3 rounded">
